...
```{r}
 ...
 red4 <- readRDS("Maestros/AllFish_FiltSec1Speed.RData")
```

### High Number of Detections
- plot shows tracks for fish with the top 10% of recorded positions, colored by hour of day 
```{r, echo=FALSE, warnings=FALSE}
   ...
   highN.red = red.npf[red.npf$npos.red >= quantile(red.npf$npos.red, .90),]
   ggplot(data=red4[red4$Id %in% highN.red$Id,], aes(x=east, y=north)) +
     ...
     ggtitle("Highest 5% N positions, positions filtered HPEs<0.5") + 
     theme(axis.text.x = element_text(angle = 90, hjust = 1))
   
   # strange looking tracks
   highn.fish = c(36379, 36483, 36612, 36632, 36675)
```
### Most of these tracks with many detections look normal; only 36379, 36483, 36612, 36632, 36675 attract my attention
- from tagID 38794 and 38823 (both noted from first two releases, last three not examined) I feel that the excessive speed filter might not be quite tight enough? 
...
```{r}
dt_threshold = 6*3600  # 6 hours; not biologically meaningful
...
- Tag 38902 absent 2.8 days before returning to array:
-- last detection of burst 38902.1 @ 2016-03-07 19:42:22; first of burst 38902.2 @ 2016-03-10 15:55:52 

- Tag 36483 not split b/c considered a predator and removed entirely. 
```{r}
...
```{r, echo=FALSE}
 ...
 f_river3 <- fortify(river3, region="BYDEL")
   ...
   full= ggplot(data=red5[red5$Id %in% c(36379,36483,36675),], aes(x=east, y=north)) +
      geom_path(data = red5[red5$burst==36675.1,], color="#440154FF") + 
      geom_path(data = red5[red5$burst==36379.1,], color="#31688EFF") + 
      geom_path(data = red5[red5$burst==36483.4,], color="#35B779FF") + 
...
## Remove the three likely predators identified holding within the array
```{r}
    red5 = red5[!(red5$Id %in% c(36379, 36483, 36675)),]  
```    
