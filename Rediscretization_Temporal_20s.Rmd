...
```{r evaluate short bursts, echo=FALSE}
...
```{r remove short bursts, echo=FALSE}
  bursts.rem = data.frame(ndetects.burst[ndetects.burst$ndet<10,])
     nrow(bursts.rem) # 217 bursts removed
    sum(bursts.rem$ndet) # 1154 positions removed
...
## Add sac river stage from CDEC
```{r add river stage, echo=FALSE}
...
```{r}
    dim(red8.trdz) # 19170 detections after discretization
    length(unique(red8.trdz$id))  # 643
    ndetects.discr = summarize(group_by(red8.trdz, id), ndet = n())  
      mean(ndetects.discr$ndet) # 30.2 per fish
      range(ndetects.discr$ndet) # ranges from 4 - 207
    max(red8.trdz$spd_mps, na.rm=T)  # 2.72 mps
    ```
