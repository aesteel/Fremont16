...
```
## These analyses now consider all five releases, both Phase 1 (spatial) and Phase 2 (survival)
...
```{r review pre-disc data, echo=TRUE}
  options(digits=20) #keep 
 red7 <- readRDS("Maestros/AllFish_FiltSec4Bursts.RData")  # single object, named red7
 
 dim(red7) # 102571 detections 
    length(unique(red7$id))  # 641 unique fish
    length(unique(red7$burst))  # 1161 unique bursts
    
    ndetects.fish = summarize(group_by(red7, id), ndet = n())  
      mean(ndetects.fish$ndet) # 160.0 per fish
      range(ndetects.fish$ndet) # ranges from 3 - 578
    ndetects.burst = summarize(group_by(red7, burst), ndet = n())  
      mean(ndetects.burst$ndet) # 88.3 per fish
      range(ndetects.burst$ndet) # ranges from 3 - 578
      
    max(red7$spd_mps, na.rm=T)  # 67.6 mps
```

```{r short bursts, echo=FALSE}
...
```{r spat disc, echo=TRUE}
  # discretize in space
  redlt.ssrdz = (redisltraj(red8.ltraj, u=27, type="space", nnew=50)) 
    # u=25 is the smallest distance where the code will work, when ran with rel 1&2
    # u=27 is the smallest distance where the code will work, when ran with rel 1-5
  ...
  # recalculate migration speed  
   red8.ssrdz$spd_mps = red8.ssrdz$dist / red8.ssrdz$dt
    ...
    dim(red8.ssrdz) # 12257 detections after discretization
    length(unique(red8.ssrdz$id))  # 634
    ndetects.discr = summarize(group_by(red8.ssrdz, id), ndet = n())  
      mean(ndetects.discr$ndet) # 19.3 per fish
      range(ndetects.discr$ndet) # ranges from 2 - 27
    max(red8.ssrdz$spd_mps, na.rm=T)  # 20.5 mps
...
#    relstg = stage[,c("DatePST","datetimePST","Stage_ft","Vel_fps","MeasQ_cfs")]
  ...
  hist(compare$stage_ft.x - compare$stage_ft.y, xlab="point estimates (USGS stage - CDEC stage)", main="compare spot measures from USGS to continuous gage of CDEC")
 ...
 ## merge onto position dataset
  ...
  ## write this to file
  saveRDS(red8.ssrdz, "Maestros/RediscSpat_27m.RData")
```
__writes discretized dataset to file: "Maestros/RediscSpat_27m.RData"__
