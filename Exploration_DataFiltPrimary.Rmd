...
``` {r, echo=FALSE}
 alldf = readRDS("Maestros/AllPos1to5.RData")          # releases 1 & 2, combined with releases 3, 4, & 5; saved as R-object
...
``` {r, echo=FALSE}
 ...
 alldf <- alldf[alldf$Id<65000,]  # matches VEMCO in number of detections; one tag is present in both sets of releases
...
``` {r, echo=FALSE, warning=FALSE}
   allsync12 <- read.csv("C:/Users/Anna/Documents/03_SacBankUSACE/FremontWeir2016/YBUS-DataFromUSGS_Sept2016/Data/Acoustic.Telemetry.Data/VEMCO.2D positioning.data.8-11-16/Analysis Package 1/Positions_RelEv12/AllSyncRef.csv")
   allsync345 <- read.csv("C:/Users/Anna/Documents/03_SacBankUSACE/FremontWeir2016/YBUS-DataFromUSGS_Sept2016/Data/Acoustic.Telemetry.Data/VEMCO.2D positioning.data.8-11-16/Analysis Package 3/Positions_RelEv345/AllSyncRef.csv")
   allsync = rbind(allsync12, allsync345)

   options("digits"=15)
   calibsync <- read.csv("C:/Users/Anna/Documents/03_SacBankUSACE/FremontWeir2016/YBUS-DataFromUSGS_Sept2016/Data/Acoustic.Telemetry.Data/VEMCO.2D positioning.data.8-11-16/Analysis Package 1/CalibratedStationPositions.csv")
    #   calibsync345 <- read.csv("C:/Users/Anna/Documents/03_SacBankUSACE/FremontWeir2016/YBUS-DataFromUSGS_Sept2016/Data/Acoustic.Telemetry.Data/VEMCO.2D positioning.data.8-11-16/Analysis Package 3/CalibratedStationPositions.csv")   # the calibrated positions were identical
    # calibsync = rbind(calibsync12, calibsync345)  
  ...
  # median error for all positions
   median(allsync.calib$error_m)

  # look at errors for rel 1,2 and rel 3,4,5 ##
   allsync.calib$Time = as.POSIXct(allsync.calib$Time, tz="GMT")
    allsync.calib$RelWindow = NA
     allsync.calib$RelWindow[allsync.calib$Time < as.POSIXct("2016-03-09", tz="GMT")] <-  12
     allsync.calib$RelWindow[allsync.calib$Time >= as.POSIXct("2016-03-09", tz="GMT")] <- 345
     
    # rel 1/2  
    median(allsync.calib$error_m[allsync.calib$RelWindow==12])
    # rel 3/4/5
    median(allsync.calib$error_m[allsync.calib$RelWindow==345])
   
  # median errors by fixed tag - all data
    tagmed = allsync.calib %>%
      group_by(Id) %>%
      summarize(med.error = median(error_m), Lat = mean(Latitude), Long = mean(Longitude)) %>%
      data.frame()
    
    ggplot(tagmed[tagmed$Id!=65106,], aes(x=Long, y=Lat, color=med.error)) +
      geom_point(cex=4) + coord_fixed() + scale_color_viridis() + 
      #geom_path(data = ggriverbank, aes(x=long, y=lat, group=group)) +   
          # I can add river if I don't add color grouping variable above. No time to solve. Nov 3, 2016
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
      scale_x_continuous(breaks=seq(-121.672, -121.664, 0.001)) +
      ggtitle("Median Measured Error (m) for fixed location tags; \necluded tagID 65106 with error = 29.2m")

  # median errors by fixed tag - release event
    tagmed.relev = allsync.calib %>%
      group_by(Id, RelWindow) %>%
      summarize(med.error = median(error_m), Lat = mean(Latitude), Long = mean(Longitude)) %>%
      data.frame()
    
    ggplot(tagmed.relev[tagmed.relev$RelWindow==12,], aes(x=Long, y=Lat, color=med.error)) +
      geom_point(cex=4) + coord_fixed() + scale_color_viridis() + 
      #geom_path(data = ggriverbank, aes(x=long, y=lat, group=group)) +   
          # I can add river if I don't add color grouping variable above. No time to solve. Nov 3, 2016
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
    ...
    ggplot(tagmed.relev[tagmed.relev$RelWindow==345,], aes(x=Long, y=Lat, color=med.error)) +
      geom_point(cex=4) + coord_fixed() + scale_color_viridis() + 
      #geom_path(data = ggriverbank, aes(x=long, y=lat, group=group)) +   
          # I can add river if I don't add color grouping variable above. No time to solve. Nov 3, 2016
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
      scale_x_continuous(breaks=seq(-121.672, -121.664, 0.001)) +
      ggtitle("Median Measured Error (m) for fixed location tags,\nRelease Events 3, 4, & 5")
...
- Based on these graphics I lean towards using an HPE filtering threshold of HPE=1; this seems to be a nice tradeoff between precision (mean = 6m in HPE bin 0.75-1) and quantity of data (~80% of data retained). 
- After incorporating the later three releases at higher flows, it seems that eprhaps a threshold of HPE<0.5 is more suitable for the following reasons:
...
- The vemco reports are a little more difficult to interpret in 2016 because they were completed in two rounds. For release 1&2, at HPE<0.5 68% of fish positions were retained, and 50% had errors < 1.57m while 95% had errors < 4.62. This is more comparable to 2015 than using a threshold of HPE<1 (50% = 1.68, 95% = 5.86). However, for releases 3, 4, & 5, at HPE<0.5 64% of positions are retained (similar), yet the errors go up (much higher flows). 50% had errors < 2.04, 95% had errors < 10.74, and 90% had errors < 7.24. Eesh. 
...
```{r}
  ...
  percKeep <- (nKeepHpe/(nLowHpe+nHighHpe+nKeepHpe))*100
    ...
    highhpe=ggplot(data=alldfg[alldfg$Hpes>=5 & alldfg$Hpes<50,], 
                   aes(y=Latitude, x=Longitude, colour=Hpes)) + 
      geom_point(size=1.5) + 
      geom_polygon(data = ggriverbank, aes(x=long, y=lat, group=group)) + 
      scale_colour_gradient2(name="HPEs", low = "green3", mid="yellow", high = "red", 
                             midpoint=15, space = "rgb", na.value = "grey50", guide = "colourbar") +
      ggtitle(paste0("Positional HPEs > 5: ",round(percHigh,1),"% of all positions")) +
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
      scale_x_continuous(breaks=seq(-121.672, -121.664, 0.001)) +
      coord_fixed() + 
      theme_bw() + theme(title=element_text(vjust=2),
...
#             commonTheme
    ...
    lowhpe=ggplot(alldfg[alldfg$Hpes<5 & alldfg$Hpes>=0.5,], aes(y=Latitude, x=Longitude, colour=Hpes)) + geom_point(size=1.5) + 
      scale_colour_gradient2(name="HPEs", low = "green3", mid="yellow", high = "red", midpoint=3, space = "rgb", na.value = "grey50", guide = "colourbar") +
      ggtitle(paste0("Positional HPEs from 0.5 to 5: ",round(percLow,1),"% of all positions")) +
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
      scale_x_continuous(breaks=seq(-121.672, -121.664, 0.001)) +
      coord_fixed() + 
      theme_bw() + theme(title=element_text(vjust=2), axis.title.x=element_text(vjust=-.5),axis.title.y=element_text(vjust=1))
    
    keephpe=ggplot(alldfg[alldfg$Hpes<0.5,], aes(y=Latitude, x=Longitude, colour=Hpes)) + geom_point(size=1.5) + 
      scale_colour_gradient2(name="HPEs", low = "green3", mid="yellow", high = "red", midpoint=.8, space = "rgb", na.value = "grey50", guide = "colourbar") +
      ggtitle(paste0("Positional HPEs < 0.5: ",round(percKeep,1),"% of all positions")) +
      scale_y_continuous(breaks=seq(38.758,38.764,0.001)) + 
      scale_x_continuous(breaks=seq(-121.672, -121.664, 0.001)) +
      coord_fixed() + 
      theme_bw() + theme(title=element_text(vjust=2), axis.title.x=element_text(vjust=-.5),axis.title.y=element_text(vjust=1))
    
    png("Graphics/Figure9_HPE_Spatial_Distn_alldat.png", width=6, height=9, units="in", res=600)
   ...
   ggplot(ggriverbank) +  aes(long,lat,group=group, fill = id) +
    ...
    roundfunc <- function(l) { (l <- round(l, 3))}  
     commonTheme = list(labs(color="Density",fill="Density",
        ...
        # print plots in propwer grid
        windows()
        plot_grid(ptdens.remove, ptdens.keep, labels = c("A", "B"), ncol=1, align="v")
        #plot_grid(ptdens.remove, ptdens.keep, labels = c("D", "E"))        
        
```    


###  Implement HPEs threshold in filtering
- This now matches the report from VEMCO (they report 83,658 + 21,054 = 104712 positions or 67% after filtering to HPE<0.5)
```{r, echo=FALSE, warnings=FALSE}  
  reddf = alldfg[alldfg$Hpes < 0.5,]
   print("After filtering @ HPEs<0.5, dataset retained:")
   print(paste0("  ",nrow(reddf)," positions,")) # 104712
   print(paste0("  ",round(nrow(reddf)/nrow(alldfg)*100,2), "% of fish tag positions"))  # 67.15%
   print(paste0("  ",length(unique(reddf$Id))," individual fish"))  # 644
...
Here is an excerpt from VEMCO's results document for releases 1 & 2 that is returned with the post-processed VPS dataset, regarding thier estimate of the precision of the fish positions:
...
And the numbers from VEMCO's results document for releases 3, 4, & 5:
> The following is a summary of these analyses:
> HPEs  # Pos   % pos   50%ile   80%ile   90%ile   95%ile
> 0.2    4527     13%    1.83     3.36     5.00      6.59
> 0.5   21054     64%    2.04     4.58     7.24     10.74
> 1.0   28279     86%    2.22     5.49     9.03     13.39
For example, using an HPEs filter of 1.0, half of the pairs of calculated positions that are within 2.0 seconds of each other are less than 2.22 metres apart, and 80% are within 5.49m.
***
...
```{r, echo=FALSE, fig.height=8, fig.width=7}
 ...
 all.npf = alldf %>%
  ...
  png("Graphics/Figure8a_HPE_PosPerFish.png", width=6, height=5, units="in", res=600)
   hist(all.npf$npos.all, xlim=c(0,1000), breaks=250, ylim=c(0,110),
    ...
    text(x=800, y=40, "Removed one individual \nw/ > 5000 detections", font=3)
  dev.off()

  outliers.pre = all.npf[all.npf$npos.all %in% sort(boxplot.stats(all.npf$npos.all)$out),]
      
  ggplot(data=alldf[alldf$Id %in% outliers.pre$Id,], aes(x=Longitude, y=Latitude)) + geom_point() +
    facet_wrap(~Id, ncol=6) + ggtitle("Tracks of outliers, positions not filtered")
 ...
 red.npf = reddf %>%
  ...
  png("Graphics/Figure8b_HPE_PosPerFish.png", width=6, height=5, units="in", res=600)
    hist(red.npf$npos.red, xlim=c(0,1000), breaks=40, ylim=c(0,110), 
         xlab=c("Number of Positions"), main="Positions recorded per fish\nafter HPE filtering")  
     text(x=800, y=40, "No individuals removed", font=3)
  dev.off()
...
### Look at fish which >60% positions removed
- how many are there?
+ 19 fish
- are the tracks indicative of predator behavior?
+ not especially, but a couple may raise interest (Id# 36483 & 39613)
- where are the errors?
+ there is a noticable location about 3/4 through the array where many of these tracks have gaps after filtering
```{r, echo=FALSE, warnings=FALSE}
  # how many?      
   poorHpes = comp.npf[comp.npf$perc.remain<=.4,]  # 19 fish
      print(paste0("Only ",nrow(poorHpes)," fish with >60% of positions removed"))
  ...
  # does the track change significantly when the poor Hpes points are removed? Not really
  ...
  poor.post = ggplot(data=alldfg[alldfg$Id %in% poorHpes$Id & alldfg$Hpes<1,], aes(x=Longitude, y=Latitude)) + geom_point() + facet_wrap(~Id, ncol=4) + ggtitle("'Poor' tracks filtered at HPEs < 0.5")
