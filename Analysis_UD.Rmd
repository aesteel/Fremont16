...
```{r kud 16 all fish, echo=FALSE}
  ...
  # create kud using lscv and save to file   
    ...
    # saveRDS(kud16.lscv_g10, "Maestros/kud16_lscv_stag.RData")
     
  # read in kud with lscv method   
     kud16.lscv = readRDS("Maestros/kud16_lscv_stag.RData") 
  ...
  # create kud with href and set h value         
    ...
    kud16.h15 = kernelUD(red8.grp[,1], h=3.03, kern="bivnorm", extent=0.1, grid=grid10) 
      # grid of 2x2 
      # kud16.grid2 = kernelUD(red8.grp[,1], h=3.03, kern="bivnorm", extent=0.1, grid=grid2) 
    
    kud16 = kud16.h15
```

## Convert kernel UDs into volume UDs for plotting; then plot the three different smoothing parameters
...
#     vud16.grid2 <- getvolumeUD(kud16.grid2) 
#       xyz.grid2 <- as.image.SpatialGridDataFrame(vud16.grid2[[1]])

  png("Graphics/kernelUD_heatmap16methods.png", width=10, height=3, units="in", res=600)
      par(mfrow=c(1,3))
     image(vud16.lscv[[1]], col=c(heat.colors(6),"white"), main="LFC 2016; h=LSCV")
      contour(xyz.lscv, add=TRUE, nlevels=6, drawlabels=FALSE)  
         plot(river3, add=T, col="grey60")
     image(vud16.href[[1]], col=c(heat.colors(6),"white","white"), main="LFC 2016; h=HREF")
...
```{r kud 16 by release, echo=FALSE} 
  ...
  print(data.frame(summarize(group_by(red8.trdz, grp), mn.stg=mean(stage_ft), sd.stg=sd(stage_ft), max.stg=max(stage_ft), min.stg=min(stage_ft), nfish=length(unique(id)))))
    ...
    kud16.g1 = kernelUD(redgrp1[,1], h=3.03, kern="bivnorm", extent=0.1, grid=grid10) # set same as lscv, 2015
     vud16.g1 <- getvolumeUD(kud16.g1) 
     xyz.g1.16 <- as.image.SpatialGridDataFrame(vud16.g1[[1]])

    kud16.g2 = kernelUD(redgrp2[,1], h=3.03, kern="bivnorm", extent=0.1, grid=grid10) # set same as lscv, 2015
   ...
   png("Graphics/kernelUD_heatmap1516grp1.png", width=7, height=4, units="in", res=600)     
    par(mfrow=c(1,2))
     image(vud15[[1]], col=c(heat.colors(6),"white"), main="LFC 2015; h=3.03")
      contour(xyz.15, add=TRUE, nlevels=6, drawlabels=FALSE)
            plot(river3, add=T, col="grey60")
     image(vud16.grp1.h15[[1]], col=c(heat.colors(6),"white"), main="LFC 2016, rel1; h=3.03")
...
```{r comp contour 15 16 release, echo=FALSE, fig.height=4, fig.width=8}
  # heatmap of 2016-rel1 vs 2016-rel2
   png("Graphics/kernelUD_heatmapgrp12.png", width=7, height=4, units="in", res=600) 
    par(mfrow=c(1,2))
     image(vud16.grp1.h15[[1]], col=c(heat.colors(6),"white"), main="LFC 2016, Group 1; h=3.03")
      contour(xyz.g1.16, add=TRUE, nlevels=6, drawlabels=FALSE)  
            plot(river3, add=T, col="grey60")
     image(vud16.grp2.h15[[1]], col=c(heat.colors(6),"white"), main="LFC 2016, Group 2; h=3.03")
...
#   grid16.grid2.kud = data.frame(cbind(kud16.grid2$LFC@coords, kud16.grid2$LFC@data$ud))
#     names(grid16.grid2.kud) <- c("Easting","Northing","UD")
#   grid16.grid2.vud = data.frame(cbind(vud16.grid2$LFC@coords, vud16.grid2$LFC@data$n))
#     names(grid16.grid2.vud) <- c("Easting","Northing","volUD")
#    grid16.grid2.ud = merge(grid16.grid2.kud, grid16.grid2.vud)
