...
``` {r read in data}
  dat <- readRDS("Maestros/AllFish_FiltSec3Hold.RData") 
...
- Referenced from: http://aa.usno.navy.mil (mean for range of all detections, rel 1-5: 2/22 - 3/19/2016)
```{r sunrise-sunset}
  sunrise = 06.49
  sunset = 18.07
```
...
```
```{r arrival and passage time, echo=FALSE}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8}
    ...
    hrday.relev =  ggplot(data = fl.df2, aes(x=Rel.hrDayfac, y=Delay.hr, fill=factor(RelEv))) + 
      geom_boxplot(outlier.shape=NA) + 
      ylim(25,55) + 
...
```
- note: consider creating this second set of boxplots in conjunction with a hydrograph to illustrate relationship between transit time and stage; from these boxplots that seems like it might be the primary driver
...
#    relcol = c("grey30","grey70")
     fl.dfplot = fl.df2
     fl.dfplot = fl.dfplot[order(fl.dfplot$Rel.hrDayfac),]
     fl.dfplot$groupid = paste(fl.dfplot$RelEv, fl.dfplot$Rel.hrDayfac, sep="-")
      groupid = unique(fl.dfplot$groupid)
     
    par(mfrow=c(5,4), mar=c(0,0,1,0))
    for(i in groupid)  
     { plotdat = suppressWarnings(as.circular(fl.dfplot$F.hrDay[fl.dfplot$groupid==i],
                              units="hours", 
                              template="clock24", 
                              rotation="clock"))
       metadat = fl.dfplot[fl.dfplot$groupid==i,]
       plotlabels = unique(paste0("Release Event ",metadat$RelEv,
                                  " - ",metadat$Rel.hrDayfac,":00"))
       ...
       rose.diag(plotdat, bins=24, cex=.8, prop=1.1, 
                 #col=relcol[unique(metadat$RelEv)], 
                 add=TRUE)
    }
```
...
## Initial look at anova and diagnostics for the lm to assess relationship between release event (1-5) and release time on transit time
##*Can we evaluate fish released at different times all together, or do we need to seperate them out?*
Indicates that only release event is important, but **doesn't meet assumptions of normality**.
Also, doesn't include the reality that the release time is a circular value. 
```{r initial models, fig.width=4, fig.height=3, fig.show="hold"}
  summary(aov(fl.df2$Delay.hr~factor(fl.df2$RelEv) * fl.df2$Rel.hrDay))
  summary(lm(fl.df2$Delay.hr~factor(fl.df2$RelEv) * fl.df2$Rel.hrDay))
   mfrow=c(2,2); plot(lm(fl.df2$Delay.hr~factor(fl.df2$RelEv) * fl.df2$Rel.hrDay))
...
*Depending upon the nature of the data, we can or can't use some of the following circular statistical tests*
...
I tried to use the guidance in Pewsey 2013 textbook to fit a cosine regression model, but the data on delay time are too skewed for it to fit the assumptions. Additionally, I'm not sure it's clear what the model will tell you?
...
```{r}     
   ...
   omega = 2*pi/24
   cosvar = cos(omega*fl.df2$Rel.hrDay)
   sinvar = sin(omega*fl.df2$Rel.hrDay)
   
   delaymod = lm(fl.df2$Delay.hr ~ fl.df2$RelEv + cosvar + sinvar)
   summary(delaymod)
...
```{r, echo=FALSE}
   ...
   delay2mod = lm(fl.df2$Delay.hr ~ fl.df2$RelEv + cosvar + sinvar + cos2var + sin2var)
   summary(delay2mod)
```
...
```{r, echo=FALSE}   
  ...
  # cos2var = cos(2*omega*fl.df4$Rel.hrDay)
   ...
   delay4mod = lm(fl.df4$Delay.hr ~ fl.df4$RelEv + cosvar + sinvar)
    summary(delay4mod)
...
```
Hm. Looks better, closer to homoscedasticity of residuals and no points with excessive leverage, but normality of residuals still seems pretty far off. 
   
   
### If we don't trust the linear modeling approach, we can try the Jammalamadaka-Sarma (circular) correlation coefficient to assess the reltionship between release time and arrival time (derivatives of transit time)
This is a circular version of the PEarson's product moment correlation, and programed into the 'circular' package
```{r circ cor test}
  relhrcirc = as.circular(fl.df2$Rel.hrDay, units="hours", template="clock24", rotation="clock")
  F.hrcirc = as.circular(fl.df2$F.hrDay, units="hours", template="clock24", rotation="clock")
    cor.circular(F.hrcirc, relhrcirc, test=T) 
```

### And then use the non-parametric Kruskal-Wallis rank sum test to assess the relationship between transit time (lienar continuous var) and release event (linear factor). 
Often can use the Nemenyi test for multiple comparisons after the kruskal test, but Zar notes that this is only appropriate with equal sample sizes across groups, and we have ~200 in Rels 1 & 2, 131 in Rel 3, and ~40 in Rels 4 & 5. 
Use the Dunn test instead
```{r RelEv vs delay}
  kruskal.test(fl.df2$Delay.hr~factor(fl.df2$RelEv))
    
    # posthoc test:
    library(FSA)
    dunnTest(fl.df2$Delay.hr ~ factor(fl.df2$RelEv), method="bh")
```
We see that releases 3, 4, and 5 are not different in thier delay times, but rel 1 and 2 are each unique. 

______________________________________________


## The release events appear correlated with river stage - does stage a better predictor of the differences between groups?
...
```{r, echo=FALSE}
 # SacRiver Stage - pulled from CDEC - ask for QAQC data from DWR if this seems okay 
  ...
  plot(fl.df2$Delay.hr ~ fl.df2$stage_ft)
   ...
   #plot(lm(fl.df2$Delay.hr ~ fl.df2$stage_ft))
```
...
```
The fit is very significant with the releases 3, 4, & 5 (p<0.0001) whereas without these releases it had been (p=0.83). But the diagnistic plots still don't look good, primarily the normality of residuals. 
...
## Based on the plots and the models (both those that that violate assumptions and the less powerful non-parametric models), the big picture that emerges is that the fish take ~30-50 hours to transit the 55.1 km between the release and the array, and this doesn't change based on the time of day they enter the river. But there is an important effect of release event on transit time, which is most likely driven by the differences in river stage across release events. **Therefore, I will continue with the analysis, ignoring the time of release but including the release event as a covariate (unless it is colinear with stage/dischage).**
