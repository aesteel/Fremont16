...
## Read in non-rediscretized data (just filtered) and add release metadata
``` {r}
...
```{r, echo=FALSE}
  # all fish (matching 2015 report)
   ...
   summarize(median.F.hr = median.circular(suppressWarnings(circular(F.hrDay, 
            ...
            sd.F.hr=circadian.sd(F.hrDay)[2]) %>%
   data.frame()
...
```{r, echo=FALSE}
   ...
   # create loop to run test for each release hour
    Relhrfac = sort(unique(as.numeric(as.character(fl.df3$Rel.hrDayfac))))
    result_table = data.frame(Relhrfac, teststat = rep(NA,8), pval = rep(NA,8))
    for(i in 1:8)  
