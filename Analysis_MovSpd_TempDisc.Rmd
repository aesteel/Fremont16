...
```{r plot spds, echo=FALSE}
  ...
  ## plot speed by RelEv
  sog.relev = ggplot(data=pathspd, aes(x=factor(RelEv), y=mps)) + 
    geom_boxplot() + xlab("Release Event (2016)") + ylab("Mean track speed (mps)")
  sog.relev
 ...
 ggplot(data=pathspd, aes(x=mps, group=factor(RelEv), fill=factor(RelEv))) + 
  ...
  sog.stg = ggplot(pathspd, aes(y=mps, x=mnStg, colour=factor(RelEv))) + 
    geom_point(pch=16, cex=4) +
    xlab("Mean River Stage (ft)") + ylab("Mean track speed (mps)") + 
...
```{r final model}
  ...
  fullmod = lm(mps ~ factor(RelEv) + sinrelhr + cosrelhr + mnStg, data=pathspd)
    summary(fullmod)
