...
- Tabulate same metrics for releases 1&2 and 3-5 seperately, b/c of differences in flow during those periods, and because they were processed seperately by VEMCO (even though I'll lump them here for filtering and analysis)
``` {r}
...
``` {r, echo=FALSE}
 ...
 fishrel = read.csv("C:/Users/Anna/Documents/GitHub/Fremont16/Maestros/TagReleaseList.csv", 
    ...
    # not enough memory to execute 'merge()' so using 'join' from plyr  
    library(plyr)
      alldf = plyr::join(alldf, fishrel, by="Id", type="left")
    detach("package:plyr")  
 ...
 ## RELEASES 1 & 2
  # N fish & total N positions
 print(paste0("Prior to filtering, ",nrow(alldf[alldf$RelEv%in%c(1,2),])," total positions in Releases 1 & 2"))
 print(paste0("Prior to filtering, ",length(unique(alldf$Id[alldf$RelEv%in%c(1,2)]))," individual fish positioned from Releases 1 & 2"))

  # N pos per fish
  all.npf = alldf[alldf$RelEv%in%c(1,2),] %>%
   group_by(Id) %>%
   summarize(npos.all=n())%>%
   data.frame()

  print("Summary of N positions per fish from Releases 1 & 2, after reducing to applicable data")
  summary(all.npf$npos.all)
  
  
  ## RELEASES 3 - 5 
   # N fish & total N positions 
 print(paste0("Prior to filtering, ",nrow(alldf[alldf$RelEv%in%c(3,4,5),])," total positions in Releases 3, 4, & 5"))
 print(paste0("Prior to filtering, ",length(unique(alldf$Id[alldf$RelEv%in%c(3,4,5)]))," individual fish positioned from Releases 3, 4, & 5"))

 # N pos per fish
  all.npf = alldf[alldf$RelEv%in%c(3,4,5),] %>%
   group_by(Id) %>%
   summarize(npos.all=n())%>%
   data.frame()

  print("Summary of N positions per fish from Releases 3, 4, & 5, after reducing to applicable data")
  summary(all.npf$npos.all)
```
...
### To see an assessment of error of sync tags, refer to full script "Exploration_DataFiltPrimary.R"

```{r echo=FALSE}
  alldfg = alldf # relict from old code
...
```{r, echo=FALSE}
  reddf <- readRDS("Maestros/AllFishPrimaryFilt.RData") # object called 'reddf'
